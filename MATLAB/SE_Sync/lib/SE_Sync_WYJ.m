function [SDPval, Yopt, xhat, Fxhat, SE_Sync_info, auxiliary_data] = SE_Sync_WYJ(auxiliary_data, manopt_options, SE_Sync_opts, Y0)
...
manopt_data.cost = @(Y) evaluate_objective_WYJ(Y', auxiliary_data);
manopt_data.egrad = @(Y) Euclidean_gradient_WYJ(Y', auxiliary_data)';
manopt_data.ehess = @(Y, Ydot) Euclidean_Hessian_vector_product_WYJ(Y', Ydot', auxiliary_data)';
...
%     checkgradient(manopt_data);
%     checkhessian(manopt_data);
    ...
    NablaF_X = Euclidean_gradient_WYJ_X(Yopt, auxiliary_data);
    NablaF_X = 0.5 * (NablaF_X + NablaF_X');
    X = Yopt'*Yopt;
    K_X = X*NablaF_X;


    Lambda_blocks = zeros(auxiliary_data.d*auxiliary_data.n, auxiliary_data.d*auxiliary_data.n);
    for k = 1:auxiliary_data.n
        imin = auxiliary_data.d*(k-1) + 1;
        imax = auxiliary_data.d*(k-1) + auxiliary_data.d;
        
        B_X = K_X(imin:imax, imin:imax);
        Lambda_blocks(imin:imax, imin:imax) = .5 * (B_X + B_X');
    end
    S = NablaF_X - Lambda_blocks;
    Lambda = S - NablaF_X;


    [v, lambda_min]  = eigs(S, 1, 'SA');
    rankY = rank(Yopt, 1e-3);

     if (lambda_min > SE_Sync_opts.min_eig_threshold)
        disp(sprintf('Found second-order critical point! (min eigenvalue = %g, threshold = %g)\n', lambda_min, SE_Sync_opts.min_eig_threshold));
        break;
    else
        ...
        disp(sprintf(' (min eigenvalue = %g, rank of Yopt = %g）\n', lambda_min, rankY));
        ...
        Ydot =  vertcat(zeros(r, auxiliary_data.d * auxiliary_data.n), v' * auxiliary_data.n);
        
%         Compute the directional derivative of F at Yplus along Ydot
        dF0 = trace(Euclidean_gradient_WYJ(Yplus, auxiliary_data)*Ydot');
        if dF0 > 0
            Ydot = -Ydot;
        end
...
%         cost1 = evaluate_objective_WYJ(Yopt, auxiliary_data);
%         cost2 = evaluate_objective_WYJ(Y0T', auxiliary_data);
%         if(cost1 < cost2)
%             cost1
%             cost2
%         end
...
Fxhat = evaluate_objective_WYJ(Rhat, auxiliary_data);
